...
import { FileSpreadsheet, Check, Download, Files, FolderOpen, Terminal } from 'lucide-react'
...
function PanelDescargas() {
  ...
  const handleDescargarSeleccionados = async () => {
    ...
    try {
      ...
      await window.api.iniciarDescarga(ids, carpeta, headless)
    } finally {
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-900 via-blue-950 to-slate-900 p-8 flex flex-col">
      ...
      <div className="relative flex items-start justify-between mb-10">
        ...
        <Button
          ...
          disabled={seleccionados.size === 0 || !carpeta}
      ...
      <div className="relative flex items-center justify-between mb-6 p-4 rounded-xl bg-white/5 border border-white/10 backdrop-blur-sm gap-4 overflow-hidden">
        ...
        <div className="flex items-center gap-4 shrink-0">
          ...
          >
            <div className={`relative w-8 h-4 rounded-full transition-colors duration-200 ${headless ? 'bg-blue-600' : 'bg-slate-600'}`}>
              <div className={`absolute top-0.5 w-3 h-3 rounded-full bg-white transition-all duration-200 ${headless ? 'left-4' : 'left-0.5'}`} />
            </div>
          ...
          return (
            ...
            >
              ...
              <div className={`shrink-0 p-2 rounded-lg transition-colors ${
                ...
                <FileSpreadsheet size={22} className={activo ? 'text-blue-400' : 'text-slate-400'} />
              </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <span className={`text-[10px] font-bold ${activo ? 'text-blue-400' : 'text-slate-500'}`}>
                  .{ext}
                </span>
              </div>

              {/* Checkbox */}
              <div className={`shrink-0 w-4 h-4 rounded border-2 flex items-center justify-center transition-all duration-200 ${
                activo ? 'bg-blue-500 border-blue-500' : 'border-slate-600 group-hover:border-slate-400'
              }`}>
      ...
      {(descargando || logs.length > 0) && (
        <div className="relative mt-6">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-slate-400 text-xs font-medium uppercase tracking-wider">Salida del proceso</span>
            {descargando && (
          ...
          <div className="bg-black/40 border border-white/10 rounded-xl p-4 h-48 overflow-y-auto font-mono text-xs">
            {logs.map((line, i) => (
              <p key={i} className={`leading-relaxed ${line.startsWith('ERROR') ? 'text-red-400' : 'text-green-400'}`}>
